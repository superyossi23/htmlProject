"""
Display all the images in a directory. 1/2/3/4 columns ver. css not used.
...
path_select = '.'  # Output file directory.
img_dir = wd.split('\\')[-1]
...
if col_num == 1:
    ...
    for f in filelist:
        ...
        for i in range(len(f_spl)):
            if   'CH1' in f_spl[i] or 'ch1' in f_spl[i]:
                f_spl[i] = "<font color='ffff00'>" + f_spl[i]  # Yellow
            elif 'CH2' in f_spl[i] or 'ch2' in f_spl[i]:
                f_spl[i] = "<font color='ff00ff'>" + f_spl[i]  # Magenta
            elif 'CH3' in f_spl[i] or 'ch3' in f_spl[i]:
                f_spl[i] = "<font color='0000ff'>" + f_spl[i]  # Blue
            elif 'CH4' in f_spl[i] or 'ch4' in f_spl[i]:
        ...
        wrap_info.append(path_select + '/' + f)  # Image (%s)
